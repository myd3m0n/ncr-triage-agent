...
- ✅ Eval harness — **baseline 95% (19/20)** on the held-out eval set
...
src/run_eval.py        scores classifier vs eval/eval_set.json, prints accuracy + misses
...
python3 src/run_eval.py        # accuracy on the eval set

# RAG retriever — needs the .venv (chromadb) and runs from src/ so `import llm` resolves:
